...
int main(void){
    ...
    initscr();
    do {
        wybor = czyt_wybor("Opcje:", aktual_cd[0] ? rozszerzone_menu : podstawowe_menu);
        switch (wybor) {
            case 'q':
                break;
            case 'd':
...
int czyt_wybor(char *prolog, char *wybory[]) {
    ...
    int klawisz = 0;
    opcja = wybory;
    mvprintw(0,0,"debug - 1");
    while(*opcja) {
        max_wiersz++;
        opcja++;
    }
/* chroni przed skroceniem menu, gdy CD zostanie usuniete */
    if(wybrany_wiersz >= max_wiersz)
        wybrany_wiersz = 0;
    mvprintw(0,0,"debug - 2");
    czysty_ekran();
    mvprintw(0,0,"debug - 3");
...
void rys_menu(char* opcje[], int aktual_podswietl, int pocz_wiersz, int pocz_kolumna) {
    int aktual_wiersz = 0;
    char **wska_opcji;
    char *wsk_tekstu;
    ...
    while (wska_opcji){
        ...
        wsk_tekstu = opcje[aktual_wiersz];
        wsk_tekstu++;
        mvprintw(pocz_wiersz + aktual_wiersz, pocz_kolumna, "%s" , wsk_tekstu);
        aktual_wiersz++;
        wska_opcji++;
    }
    mvprintw(pocz_wiersz + aktual_wiersz +3, pocz_kolumna, "Przesun podswietlenie potem nacisnij RETURN ");
    refresh();
}
...
void czysty_ekran(){
    ...
    refresh(); 
    if(aktual_cd[0] == '\0') {
        mvprintw(0,0,"debug - 2.1");
        mvprintw(LINIA_ERR,0, "Aktualnie CD %s: %s", aktual_kat, aktual_cd);
...
void uaktual_cd(){
    ...
    touchwin(stdscr);
    do {
        mvwprintw(wsk_pod_okno,linia_ekranu++, POZ_WIERSZY_RAMKA + 2, "Sciezla %d: ", sciezka);
        clrtoeol();
        refresh();
        wgetnstr(wsk_pod_okno, nazwa_sciezki, MAX_STRING);
        dlug = strlen(nazwa_sciezki);
        if(dlug >0 && nazwa_sciezki[dlug-1] == '\n')
            nazwa_sciezki[dlug-1]='\0';
        if(*nazwa_sciezki)
            fprintf(wsk_sciezki, "%s,%d,%s\n", aktual_kat,sciezka,nazwa_sciezki);
        sciezka++;
        if(linia_ekranu > LINIE_RAMKA -1 ){
            scroll(wsk_pod_okno);
            linia_ekranu--;
        }
    }while(*nazwa_sciezki);
...
void wypisz_sciezki(){
    ...
    wsk_sciezka_pad = newpad(sciezki + 1 + LINIE_RAMKA, WIERSZ_RAMKA +1);
    if(!wsk_sciezka_pad)
        return;
    wsk_sciezki = fopen(plik_sciezek,"r");
    if(!wsk_sciezki)
        return;
    mvprintw(4,0,"Wykaz sciezek CD\n");
    /* Zapisanie informacji o sciezce w notaniku */
    while(fgets(wpis,MAX_WPIS,wsk_sciezki)){
        if(strncmp(aktual_kat,wpis,dlug_kat)==0){
            mvwprintw(wsk_sciezka_pad,op_linie++, 0, "%s", wpis + dlug_kat + 1);
        }
    }
    fclose(wsk_sciezki);
...
#define LINIA_RETURN 25
void czyt_return(){
    int znak;
    mvprintw(LINIA_RETURN,5 ,"%s", "Nacisnij Return ");
    refresh();
    while((znak = getchar()) != '\n' && znak != EOF);
}
